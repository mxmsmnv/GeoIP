...
GeoIP detects visitor location for ProcessWire templates. It uses local MaxMind GeoLite2 data first, can use IPGeolocation.io as an HTTPS fallback, supports visitor corrections, and can log lookups.

## Requirements

- ProcessWire 3.0.200+
- PHP 8.2+
- Composer and MaxMind GeoLite2 database files for local lookup; or
- an IPGeolocation.io API key for the optional HTTPS fallback
...
Local MaxMind lookup is recommended because it avoids an external request, keeps the visitor IP on your server and has no per-request API quota.

### 4. Optional IPGeolocation.io fallback

Create an [IPGeolocation.io](https://ipgeolocation.io/) API key, then open **Modules → GeoIP**:

1. enable **IPGeolocation.io fallback**;
2. enter the API key;
3. choose a short timeout (2 seconds by default).

The fallback is called only when local MaxMind lookup is unavailable or fails. It sends the requested IP address to IPGeolocation.io over HTTPS and is subject to that service's quota, terms and your site's privacy policy.
...
| HTTP timeout | External lookup timeout, clamped to 1–10 seconds |
| Static fallback country/region/city | Used only when local and HTTP lookups both fail |
...
//   'status'      => 'success',
//   'source'      => 'maxmind', // maxmind, ipgeolocation, configured, or empty
// ]
